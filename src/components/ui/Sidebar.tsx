...
import { fetchInvitations } from '@/store/slices/teamsSlice';
...
export function Sidebar() {
  ...
  const { invitations } = useSelector((state: IRootState) => state.teams);
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3">
          <div className="space-y-1">
            ...
            >
              <div className="flex items-center space-x-3">
                <span className={pathname === Path.TeamInvitations ? 'text-white' : 'text-gray-500'}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </span>
                <span className="font-medium">{RouteTitle.TeamInvitations}</span>
              </div>
              {pendingInvitationsCount > 0 && (
                <span className={`
                  inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none rounded-full
                  ${pathname === Path.TeamInvitations ? 'bg-white text-indigo-600' : 'bg-red-500 text-white'}
                  animate-pulse
                `}>

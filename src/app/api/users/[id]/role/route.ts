...
export async function PATCH(
  ...
  return withSuperAdmin(request, async (req) => {
    try {
      ...
      // Validate role
      const validRoles: UserRole[] = ['superadmin', 'admin', 'member'];
      if (!role || !validRoles.includes(role)) {
        return NextResponse.json(
          { success: false, message: 'Invalid role value', data: null },
          { status: 400 }
        );
      }
      ...
      if (req.userId === userId) {
        return NextResponse.json(
          { success: false, message: 'Cannot change your own role', data: null },
          { status: 403 }
        );
      }
      ...
      return NextResponse.json(
        { success: false, message: 'Failed to update user role', data: null },
        { status: 500 }
      );
    }
  });
